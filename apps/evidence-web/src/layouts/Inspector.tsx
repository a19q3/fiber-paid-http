...
export function Inspector() {
  ...
  const actuatorState = (() => {
    ...
    if (replayRejected) return { state: "blocked", detail: "replay blocked", service: receipt ? "executed before receipt" : "not executed", replay: "blocked", reissued: "false", health: "guard active" };
    if (ev.busy && ev.phase !== "idle") return { state: "executing", detail: "protocol step running", service: receipt ? "executed before receipt" : "awaiting delivery", replay: "not attempted", reissued: "false", health: "API connected" };
    if (receipt || ev.phase === "receipt_returned") return { state: "active", detail: "response + receipt delivered", service: "executed before receipt", replay: "not attempted", reissued: "false", health: "API connected" };
    if (ev.flow?.authorization || ev.flow?.fiberChallenge || ev.phase === "payment_settled" || ev.phase === "challenge_received") return { state: "active", detail: ev.flow?.authorization ? "payment credential ready" : "402 challenge issued", service: "awaiting delivery", replay: "not attempted", reissued: "false", health: "API connected" };
    return { state: "idle", detail: "awaiting request", service: "not executed", replay: "not attempted", reissued: "false", health: ev.status?.livePaymentEnabled ? "live Fiber ready" : "API connected" };
  })();
