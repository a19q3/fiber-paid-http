...
pub enum ServerError {
    ...
    #[error("fl402 error: {0}")]
    Fl402(#[from] fiber_mpp_fl402::Fl402Error),
    #[error("http error: {0}")]
...
fn resolve_gateway_config(config: &ServerConfig) -> Result<GatewayConfig, ServerError> {
    let secret_env = config
        .secret_env
        .clone()
        .unwrap_or_else(|| "FIBER_MPP_SECRET".to_string());
    let secret =
        std::env::var(&secret_env).map_err(|_| ServerError::Config(format!("set {secret_env}")))?;
    if secret.len() < 32 {
    ...
    Ok(GatewayConfig {
        server_id: config
            .server_id
            .clone()
            .unwrap_or_else(|| "fiber-mpp-rs".to_string()),
        upstream: config
            .upstream
            .clone()
        ...
        storage_path: sqlite_path(
            config
                .storage
                .as_deref()
                .unwrap_or("sqlite://./fiber-mpp-rs.sqlite"),
        )?,
...
fn resolve_fl402_config(config: &Fl402Config) -> Result<Fl402GatewayConfig, ServerError> {
    let root_key_env = config
        .root_key_env
        .clone()
        .unwrap_or_else(|| "FIBER_MPP_FL402_ROOT_KEY".to_string());
    let root_key = std::env::var(&root_key_env)
        .map_err(|_| ServerError::Config(format!("set {root_key_env}")))?;
    if root_key.len() < 16 {
        return Err(ServerError::Config(format!(
