...
export function readFiberE2ePreflight(env: NodeJS.ProcessEnv = process.env): FiberE2ePreflight {
  ...
  if (!env.FIBER_PAID_HTTP_SECRET || env.FIBER_PAID_HTTP_SECRET.length < 32) {
    blockers.push("Fiber live E2E skipped: set FIBER_PAID_HTTP_SECRET to a random secret of at least 32 characters");
  }
...
export function readLiveFiberEnv(env: NodeJS.ProcessEnv = process.env): LiveFiberEnv {
  ...
  return {
    ...
    secret: env.FIBER_PAID_HTTP_SECRET!
  };
}
