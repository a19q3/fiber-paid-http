...
const alerts = fs.readFileSync("deploy/prometheus/fiber-paid-http-alerts.yml", "utf8");

const checks = [
  {
    id: "prometheus_alert_rules",
    evidence: "deploy/prometheus/fiber-paid-http-alerts.yml",
    required: [
      "FiberPaidHttpGatewayReadinessFailing",
      "FiberPaidHttpGatewayHigh5xxRate",
      "FiberPaidHttpGatewayRateLimited",
      "FiberPaidHttpGatewayNoTraffic",
      "fiber_paid_http_gateway_readiness_failures_total",
      "fiber_paid_http_gateway_responses_total",
      "fiber_paid_http_gateway_rate_limit_rejections_total",
      "fiber_paid_http_gateway_requests_total"
    ],
    source: alerts
  },
  {
    ...
    required: ["/healthz", "/readyz", "/metrics", "fiber-paid-http doctor --role gateway", "incident"],
    source: runbook
  },
  {
...
const report = {
  ...
  artifacts: {
    ...
    alert_rules: "deploy/prometheus/fiber-paid-http-alerts.yml",
