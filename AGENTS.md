...
11. **`production_ready_for_fiber_method` is hard-coded `false`** in every gate report. The blockers list (`testnet Fiber E2E evidence still pending`, `operational hardening still pending`, `long-running deployment hardening still pending`) is the source of truth for "what's left" — don't set it to `true` without explicitly retiring each blocker.
